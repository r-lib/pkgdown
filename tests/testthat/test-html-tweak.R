...
test_that("tweak_reference_topic_html() works", {
  html_skeleton <- "<body><div class='ref-section'>%s</div></body>"
  build_html <- function(html_skeleton, block) {
    xml2::read_html(sprintf(html_skeleton, block))
  }

  r_block <- '<div class="sourceCode r"><pre><code>rlang::is_installed()\n</code></pre></div>'
  expect_snapshot_output(
    cat(
      as.character(
  ...
  no_info_r_block <- '<pre><code>rlang::is_installed()\n</code></pre>'
  expect_snapshot_output(
    cat(
      as.character(
  ...
  yaml_block <- '<div class="sourceCode yaml"><pre><code>url: https://pkgdown.r-lib.org/\n</code></pre></div>'
  expect_snapshot_output(
    cat(
      as.character(
