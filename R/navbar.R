...
navbar_components <- function(pkg = ".") {
  ...
  menu <- list()
  menu$reference <- menu_link(translate("Reference"), "reference/index.html")

  if (!is.null(pkg$tutorials)) {
    menu$tutorials <- menu(translate("Tutorials"),
...
navbar_articles <- function(pkg = ".") {
  ...
  if (any(pkg_intro)) {
    ...
    menu$intro <- menu_link(translate("Get started"), intro$file_out)
  }

  meta <- pkg$meta
  if (!has_name(meta, "articles")) {
    vignettes <- vignettes[!pkg_intro, , drop = FALSE]
    menu$articles <- menu(translate("Articles"), menu_links(vignettes$title, vignettes$file_out))
  } else {
    ...
    if (length(navbar) == 0) {
      ...
      menu$articles <- menu_link(translate("Articles"), "articles/index.html")
    } else {
      ...
      if (length(navbar) != length(articles)) {
        children <- c(children, list(menu_spacer(), menu_link(translate("More articles..."), "articles/index.html")))
      }
      menu$articles <- menu(translate("Articles"), children)
    }
  }
  print_yaml(menu)
}
