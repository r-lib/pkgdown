...
[![Codecov test coverage](https://codecov.io/gh/r-lib/pkgdown/branch/main/graph/badge.svg)](https://codecov.io/gh/r-lib/pkgdown?branch=main)
<!-- badges: end -->
...
pkgdown 2.0.0 includes an upgrade from Bootstrap 3 to Bootstrap 5, which is accompanied by a whole bunch of minor UI improvements. If you've heavily customised your site, there's a small chance that this will break you site, so everyone needs to explicitly opt-in to the upgrade by adding the following to `_pkgdown.yaml`:
...
At last count, pkgdown is used [by over 6,000 packages](https://github.com/search?q=filename%3Apkgdown.yml+path%3A%2F&type=Code). Here are a few examples created by contributors to pkgdown:
