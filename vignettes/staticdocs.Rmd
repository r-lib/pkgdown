...
The goal of staticdocs is to make it easy for package developers to make elegant and useful packages websites. The defaults are tailored for smaller packages and for use on GitHub (particularly with new [`docs/` directory][docs] support), but it is flexible enough to be used as part of a bigger website. There are five parts to a staticdocs site:
...
To build a staticdocs site, run `build_site()`. The function has some arguments (mostly to control where the output), but most of the configuration options are controlled by `_staticdocs.yaml`. These options include:
...
staticdocs tries them in order, which means that if you want different display for GitHub and staticdocs, you can have both `README.md` and a `index.md`
