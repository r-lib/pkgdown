...
The goal of staticdocs is to make it easy for package developers to make elegant and useful packages websites. The defaults are tailored for smaller packages and for use on GitHub (particularly with new [`docs/` directory][docs] support), but it is flexible enough to be used as part of a bigger website.

There are seven main parts to a staticdocs site:
...
## Metadata (`_staticdocs.yml`)

Metadata about the site is stored in `_staticdocs.yaml` in the root directory. This includes:
...
* A [bootswatch](https://bootswatch.com) `theme` to easily tweak the
...
The home page will be automatically generated from one of five files (in order):

1. `index.Rmd`
1. `index.md`
1. `index.html` (eventually)
1. `README.Rmd`
1. `README.md`

Looking for both `index` and `README` allows you to default to the same homepage as github, while making it possible to have a different home page if needed.

## Reference (`reference/`)

The function reference generates one page for each `.Rd` file in `man/`, placing the results in `reference/`. This is mostly a straightforward translation to HTML, along with evaluation of example and auto-linking of all code. 

staticdocs will also generate an overall index, which by default, is just an alphabetically ordered list of functions. However, the index is better with human curation because functions can be grouped by function and described en masse. To override the defaults, provide a `reference` key in `_staticdocs.yml`:
...
The `reference` should be an array of objects containing `title`, `desc` (description), and list of `contents`. Since common prefix and suffixes are often used for functional grouping, you can use the special handlers `starts_with` and `ends_with` to automatically include all functions with a common prefix or suffix. To match more complex patterns, use `matches` with a regular expression.

staticdocs will warn if you've forgotten to include any non-internal functions.

## Articles (`articles/`)

staticdocs will automatically build all `.Rmd` vignettes, including those in subdirectories, and render output to `articles/`. staticdocs will ignore the output format defined in the yaml header, and always use `html_fragment(toc = TRUE, toc_float = TRUE)`.
