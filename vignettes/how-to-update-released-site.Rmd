...
* Update pkgdown to its latest version, maybe with `pak::pak("pkgdown")`.
* Check out your default branch and `git pull`.
* Call `usethis::use_pkgdown_github_pages()`. You may want to do this even if
  you are already using pkgdown + GitHub Pages + GitHub Actions. However, you
  will need to inspect the changes and use your judgement when staging /
  discarding individual diffs. Intended benefits:
  - Update `.github/workflows/pkgdown.yaml` to the latest version from
    `r-lib/actions`. (This specific step can also be achieved with
    `usethis::use_github_action("pkgdown")` or
    `usethis::use_tidy_github_actions()`.)
  - Make sure the pkgdown site's URL appears in `_pkgdown.yml`, in the URL
    field of DESCRIPTION, and as the website associated with the GitHub repo.
...
* Make sure you're using automatic development mode, i.e. in `_pkgdown.yml`:

  ``` yaml
  development:
    mode: auto
  ```
* Modernize `_pkgdown.yml` following the instructions here:
  - <https://tidytemplate.tidyverse.org>
* Commit and push, if any of the above caused local changes.
* Scrutinize your newly published dev site and make sure things look right.
...
* Backport innovations from the future. Most likely candidates:
  ...
  Example of a useful opening move:  
