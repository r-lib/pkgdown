...
* News (if present).
* A "github" icon (<i class="fab fa-github fa-lg"></i>) with a link to your github repo (if listed in the `DESCRIPTION` url field).
...
* Provide deeper and additional hierarchical navigation (like     
...
You can set the navbar `title` and its [bootswatch styling](https://bootswatch.com/3/cosmo/#navbar) with `type`. Navbar items take `text`, `href`, and optional `icon` (using [FontAwesome](https://fontawesome.com/) icons) values. Finally, positioning of items within the navbar can be controlled with `left` and `right` sections.
