...
_Some Bootswatch templates like lux or pulse won't work out of the box because of their tall navbar._
_Add [CSS rules to prevent overlap with other elements](https://getbootstrap.com/docs/4.6/components/navbar/#placement) like [padding to the body](https://stackoverflow.com/questions/10336194/top-nav-bar-blocking-top-content-of-the-page)._

Then, when using Bootstrap 4,
you can use any variables [documented in bslib](https://rstudio.github.io/bslib/articles/bs4-variables.html) for customizing your theme!
...
If you need to store more CSS rules, some JS code, an SVG used by your CSS, etc.

* Store the files under `inst/pkgdown/assets/BS4` (if you're using Bootstrap 4) e.g. `inst/pkgdown/assets/BS4/cool-styling.css`.
* They will be copied to the root of your website folder by pkgdown (by `init_site()`).
* For JS and CSS files, reference them in a `inst/pkgdown/after-head.html` template (see the section "Other layout changes"), e.g.

```html
<link href="{{#site}}{{root}}{{/site}}cool-styling.css" rel="stylesheet">
```
...
If you want to alter the elements of the footer, refer to `?build_site` docs.
...
pkgdown uses the following whisker templates for Bootstrap 4: 

```{r, results='asis', echo=FALSE}
cat(paste0("* ", fs::path_file(fs::dir_ls(pkgdown:::path_pkgdown("templates", "BS4"))), "\n"))
```
...
* improving the contents of your documentation themselves! :wink:
