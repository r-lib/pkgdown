...
The main bslib variables you can use to create a theme are `bg`, `fg`, and `navbar-bg`.
Optionally, you can also include `primary` and `secondary`.
Below is a list of which pkgdown website elements each variable will change:

```{r echo = FALSE}
theme_vars <- tibble::tribble(
  ~Variable, ~Element, ~Description, ~Sass,
  # global theme variables, group by variable?
  "bg", "Body", "Background color", "$body-bg",
  "fg", "Body", "Text color", "$body-color",
  "navbar-bg", "Navbar", "Background color", "$navbar-bg", 
  "primary",  "Navbar", "Package name hover color", "$navbar-light-active-color ---> $navbar-light-brand-hover-color",
  "primary",  "Navbar", "Hover link color", "$navbar-light-hover-color",
  "primary",  "Navbar", "Active link color", "$navbar-light-active-color",
  "primary",  "Body (+ Sidebar)", "Link color", "$link-color",
  "primary",  "Body (+ Sidebar)", "Link hover color", "darken($link-color, 15%)", 
  "primary", "Sidebar", "Active contents text color", "$component-active-color",
  "primary", "Body", "Active copy button icon color", "$component-active-color",
  "secondary", "Sidebar", "Contents hover/active background color", "$component-active-bg",
  "secondary", "Navbar", "Pills link active background color", "$component-active-bg ---> $nav-pills-link-active-bg", 
  "secondary", "Navbar", "Dropdown link hover background color", "$component-active-bg ---> $dropdown-link-active-bg", 
  "secondary", "Body", "Copy button hover/active background color", "$component-active-bg"
)
theme_vars$Variable <- paste0("<code>", theme_vars$Variable, "</code>")
theme_vars$Sass <- paste0("<code>", theme_vars$Sass, "</code>")
...
```{r echo = FALSE}
extra_vars <- tibble::tribble(
  ~Variable, ~Element, ~Description, ~Overrides,
  # body text colors, no group?
  "body-color", "Body", "Text color", "$fg",
  "headings-color", "Body", "Headings color (h1-h6)", NA,
  "link-color", "Body", "Link color", "$primary",
  "link-hover-color", "Body", "Link hover color", "darken($link-color, 15%)",
  # sidebar / clipboard buttons
  "component-active-color", "Sidebar", "Active contents text color", "$primary",
  "component-active-color", "Body", "Active copy button icon color", "$primary",
  "component-active-bg", "Sidebar", "Active contents background color", "$secondary",
  "component-active-bg", "Body", "Active copy button background color", "$secondary",
  "component-active-bg", "Navbar", "Active dropdown link background color", "$secondary",
  "component-active-bg", "Navbar", "Active pills link background color", "$secondary",
)
extra_vars$Variable <- paste0("<code>", extra_vars$Variable, "</code>")
extra_vars$Overrides <- paste0("<code>", extra_vars$Overrides, "</code>")

reactable(extra_vars,
  groupBy = "Variable",
  defaultExpanded = FALSE,
  columns = list(
    Variable = colDef(html = TRUE),
    Overrides = colDef(html = TRUE)
  )
)
```
...
Based on your `navbar-bg` color and the navbar `type`, Bootstrap automatically calculates a color for the navbar text elements, including the navigation links, the package name (the "brand"), and active/hover states for each.
You can instead use the following bslib variables to control these values, depending on whether you choose a `light` or `dark` navbar:

```{r echo = FALSE}
navbar_vars <- tibble::tribble(
  ~Variable, ~Type, ~Description, ~Overrides,
  # group by type?
  # type = light
  "navbar-light-active-color", "light", "Active color for link text", "$primary",
  "navbar-light-contrast", "light", "Calculated to contrast with navbar-bg color", NA,
  "navbar-light-brand-color", "light", "Package name text color", "$navbar-light-active-color",
  "navbar-light-brand-hover-color", "light", "Package name hover color", "$navbar-light-active-color",
  "navbar-light-color", "light", "Link color", "rgba($navbar-light-contrast, .5)",
  "navbar-light-hover-color", "light", "Hover color for link text", "rgba($navbar-light-contrast, .75)",

  # type = dark
  "navbar-dark-active-color", "dark", "Active color for link text", "$primary",
  "navbar-dark-contrast", "dark", "Calculated to contrast with navbar-bg color", NA,
  "navbar-dark-brand-color", "dark", "Package name text color", "$navbar-dark-active-color",
  "navbar-dark-brand-hover-color", "dark", "Package name hover color", "$navbar-dark-active-color",
  "navbar-dark-color", "dark", "Link color", "rgba($navbar-dark-contrast, .5)",
  "navbar-dark-hover-color", "dark", "Hover color for link text", "rgba($navbar-dark-contrast, .75)",
)

navbar_vars$Variable <- paste0("<code>", navbar_vars$Variable, "</code>")
navbar_vars$Overrides <- paste0("<code>", navbar_vars$Overrides, "</code>")

reactable(navbar_vars,
  groupBy = "Type",
  defaultExpanded = FALSE,
  columns = list(
    Variable = colDef(html = TRUE),
...
Furthermore, if you want to tweak *which* authors of the package appear in the sidebar and footer, also refer to `?build_home`.
