...
You'll need to set the `pkgdown-nav-height` bslib variable to the height of the navbar (by default it's `56px`).
...
You can customise individual theme components using bslib variables:
...
While iterating on these variables you only need to rerun `init_site()` to re-generate the CSS files.

### Navbar colors

The navbar colours are determined by HTML classes, not CSS.
They can be customized the color scheme of the navbar by using the [`bg` and `type` parameters](https://getbootstrap.com/docs/4.0/components/navbar/#color-schemes).
You can set the `bg` to any colour, and then pick either `type: light` or `type: dark` depending on whether the background is light (and hence needs dark text) or dark (and hence needs light text.)

For example, if the primary colour is dark, you'll want to use a dark navbar:
...
Because the navbar is styled with HTML, you'll need to `build_site()` to see the effect of changing this parameter.
