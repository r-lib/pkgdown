...
-   **Layout** changes the content (or ordering) of the navbar, authors page, or `<head>` tag, by using pkgdown options or templates.
...
Most theming features work only with Bootstrap 4, so begin by update your site to use Bootstrap 4 by adding the following lines to your `_pkgdown.yml`:
...
Upgrading to Bootstrap 4 has a low of breaking your site, but if you were managing your [own pkgdown templates](#layout) or using custom CSS, you might need to tweak a few files.
...
-   Create a folder `inst/pkgdown/templates`.
-   Create `in-header.html`, `before-body.html` or `after-body.html` in that folder, depending on your needs. E.g. you could save the lines corresponding to your analytics provider, if not Google Analytics, in `inst/pkgdown/templates/in-header.html`.
...
-   Templates in `inst/pkgdown/templates/BS4` will override the default templates.
-   Assets in `inst/pkgdown/assets/BS4` will be copied in to the destination directory.
